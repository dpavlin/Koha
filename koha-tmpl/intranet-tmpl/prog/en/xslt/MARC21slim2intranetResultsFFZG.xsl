...
<!--
    ...
    <xsl:call-template name="nameABCQ"/>  <xsl:text> </xsl:text> (<xsl:value-of select="marc:subfield[@code=9]"/>)</a>
    <xsl:choose>
...
<xsl:if test="$DisplayIconsXSLT!='0'">
    ...
    <xsl:if test="$typeOf008!=''">
            <xsl:choose>
                ...
                <xsl:when test="$leader6='t'"><img src="/intranet-tmpl/prog/img/famfamfam/silk/script.png" alt="script" title="script" class="materialtype"/> Manuscript</xsl:when>
        ...
        <span class="label">; Nature of contents: </span>
		    ...
		    bibliography
                ...
                <xsl:when test="contains($controlField008-24,'m')">
                    <img src="/intranet-tmpl/prog/img/famfamfam/silk/report.png" alt="theses" title="theses" /> theses
                </xsl:when>
...
<xsl:text> </xsl:text> <!-- added blank space to fix font display problem, see Bug 3671 -->
    ...
    <span class="results_summary edition">
            ...
            <xsl:for-each select="marc:datafield[@tag=942]">
                    <xsl:call-template name="subfieldSelect">
                        <xsl:with-param name="codes">tu</xsl:with-param>
                    </xsl:call-template>
            </xsl:for-each>
    </span>
    </xsl:if>
