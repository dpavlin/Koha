...
<xsl:stylesheet version="1.0"
  ...
  exclude-result-prefixes="marc items">
    ...
    <xsl:key name="item-by-status-and-branch" match="items:item" use="concat(items:status, ' ', items:homebranch, ' ', items:location)"/>
	<xsl:key name="item-by-branch-location-callnumber" match="items:item" use="concat(items:homebranch, ' ', items:location, ' ', items:itemcallnumber)"/>
...
<xsl:text> </xsl:text> <!-- added blank space to fix font display problem, see Bug 3671 -->
    ...
    <xsl:if test="marc:datafield[@tag=856]">
         <span class="results_summary online_resources">
               ...
               <xsl:for-each select="$available_items[generate-id() = generate-id(key('item-by-status-and-branch', concat(items:status, ' ', items:homebranch, ' ', items:location))[1])]">
                               <span class="label"><xsl:value-of select="items:location"/></span>
                   <xsl:if test="items:itemcallnumber != '' and items:itemcallnumber"> [<xsl:value-of select="items:itemcallnumber"/>]</xsl:if>
                               <xsl:text> (</xsl:text>
                               <xsl:value-of select="count(key('item-by-status-and-branch', concat(items:status, ' ', items:homebranch, ' ', items:location)))"/>
                               <xsl:text>)</xsl:text>
                               <xsl:choose><xsl:when test="position()=last()"><xsl:text>. </xsl:text></xsl:when><xsl:otherwise><xsl:text>, </xsl:text></xsl:otherwise></xsl:choose>
                               </xsl:for-each>
               </xsl:when>
            ...
            <xsl:choose>
                <xsl:when test="count(key('item-by-status', 'reference'))>0">
                    <span class="available">
                        ...
                        <xsl:for-each select="$reference_items[generate-id() = generate-id(key('item-by-status-and-branch', concat(items:status, ' ', items:homebranch, ' ', items:location))[1])]">
                                <span class="label"><xsl:value-of select="items:location"/></span>
                                <xsl:if test="items:itemcallnumber != '' and items:itemcallnumber"> [<xsl:value-of select="items:itemcallnumber"/>]</xsl:if>
                                <xsl:text> (</xsl:text>
                                <xsl:value-of select="count(key('item-by-status-and-branch', concat(items:status, ' ', items:homebranch, ' ', items:location)))"/>
