...
function Focus$function_name(subfield_managed) {
	...
	if ( document.getElementById(\"$field_number\").value ) {
		...
		document.getElementById(\"$field_number\").value='$dateentered' + '|           || p  |||| |   |0   |d';
	}
    return 1;
}
...
function Clic$function_name(i) {
	...
	newin=window.open(\"../cataloguing/plugin_launcher.pl?plugin_name=ffzg-marc21_field_008_peri.pl&index=$field_number&result=\"+defaultvalue,\"tag_editor\",'width=1000,height=600,toolbar=false,scrollbars=yes');
...
sub plugin {
    ...
    $result = "$dateentered" . "|           || p  |||| |   |0   |d" unless $result;
    my $errorXml = '';
