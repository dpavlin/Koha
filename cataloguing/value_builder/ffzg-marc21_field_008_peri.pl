...
function Focus$function_name(subfield_managed) {
	if ( document.getElementById(\"$field_number\").value ) {
        ...
        document.getElementById(\"$field_number\").value='$dateentered' + 's           || p|||   ||   ||   |d';
	}
    return 1;
}
...
function Clic$function_name(i) {
    ...
    newin=window.open(\"../cataloguing/plugin_launcher.pl?plugin_name=ffzg-marc21_field_008_peri.pl&index=$field_number&result=\"+defaultvalue+leader_parameter,\"tag_editor\",'width=1000,height=600,toolbar=false,scrollbars=yes');
...
sub plugin {
    ...
    $result = "$dateentered" . "s           || p|||   ||   ||   |d" unless $result;
    my $errorXml = '';
